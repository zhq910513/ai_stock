...
状态：migrated_copy
来源：ai_stock_source/services/source-data-service
...
- Any request parameter whose key starts with `__` is orchestration metadata, including backup plans, source-build aliases and repair attempt ids. Worker execution strips these keys before calling provider adapters, so repair attempts can be uniquely audited in the queue without changing the real provider request or raw/source lineage hash.
...
- Duplicate fetch submissions that resolve to an active `queued` or `leased` raw job must register a `__source_build_aliases` entry for the current `fetch_batch_id + source_table_name + symbol + trade_date + canonical_fields`. When the shared raw job later succeeds, completion must create build triggers for both the original source table and every active alias using the alias target identity; candidate-level batches such as Model 4 Day1 daily/limit/trade-status repairs must not finish as empty duplicate skips while the original raw job is still running.
...
- Durable raw reads must expose typed raw-table columns to the quality gate, not only legacy `raw_row_json`. For `raw_ths.paid_limit_up_probability_v1`, the durable reader merges safe physical columns such as `symbol`, `trade_date`, `paid_limit_up_probability`, `status_code`, `status_msg`, `credential_version`, `available_at` and `raw_provider_row` into the source-build row payload, and derives `date` only from controlled request params or `trade_date`. This keeps the existing quality gate active while allowing successful THS paid probability raw rows to build `source.ths_paid_limit_up_probability_v1` and lineage after API/worker restarts.

## Daily Data Summary Read-Only Contract

`GET /source/ops/daily-data-summary?trade_date=YYYY-MM-DD` 返回 `source_daily_data_summary_v1`，用于 admin 日生命周期看板和只读对账。接口只读取 Postgres 持久化事实，不提交 fetch、不触发 worker、不调用 provider、不写 raw/source/lineage。

读取范围包括 `governance.raw_fetch_job_item_v1` 的当日 raw job 状态、`governance.source_build_execution_result_v1` 与 `governance.source_build_trigger_v1` 的当日 build 结果、`governance.source_gap_v1` 的当日缺口，以及 `source.*` 标准表中可按 `trade_date/trading_day/calendar_date/day/bar_time/snapshot_time/event_time/captured_at/available_at/updated_at` 识别的当日行数和最新更新时间。输出按 `source_table_name` 聚合 `raw_succeeded_count`、`raw_active_count`、`raw_waiting_count`、`raw_failed_count`、`build_succeeded_count`、`build_failed_count`、`source_row_count`、`lineage_row_count`、`failure_samples` 与 `latest_*_at`。

Daily summary keeps `raw_failed_jobs` / `raw_failed_table_count` as provider and primary/backup audit facts. Admin completion must use `data_failed_table_count` only for final asset failures: unrecovered build failure, P0 gap, or all raw attempts failed with no successful source/build output. Build results are evaluated by `source_table_name + symbol + trade_date`: if an earlier `failed/skipped_no_raw` build is followed by a successful build with source rows for the same identity, the old failure is reported as `build_failure_audit_count` / `build_failure_audit_results` and does not remain in `build_failed_count` or `build_failed_results`. When a primary provider failed but backup source/build succeeded, or when a build failure was later recovered, the table reports `data_asset_status=completed_with_provider_audit`; dashboards must treat it as an audit warning, not a final failed data asset. `source_row_count>0` or build succeeded means source output exists for the day, not that model release preflight must pass. Missing facts remain NULL/gap/failure samples; never fill with 0, mock, frontend inference or historical runs.

Full-A daily assets must also pass table-level coverage before they can be counted as final completed data. For `source.stock_universe_daily_v1`, `source.trade_status_v1`, `source.limit_price_v1`, `source.daily_bar_v1`, and `source.adjusted_daily_bar_v1`, daily summary compares the actual source rows with the same-day full-A universe baseline and requires at least 99.5% coverage, with a 5000-row floor when the universe baseline is unavailable or partial. A partial output such as a few hundred `source.limit_price_v1` rows is reported as `data_asset_status=coverage_insufficient`, `coverage_insufficient=true`, and `final_data_failed=true`; it must not be counted as completed just because `source_row_count>0`. `data_produced_table_count` counts only `completed` and `completed_with_provider_audit`, not partial coverage.

2026-07-14 performance contract: Daily summary build-result recovery must use a one-pass successful identity relation keyed by `source_table_name + symbol + trade_date`, not a per-failed-row correlated scan over all same-day build facts. Timestamp source-table counts use half-open day ranges (`>= trade_date` and `< trade_date + 1 day`) instead of casting timestamp columns to date, so existing time indexes remain usable. Source build workers must select queued build triggers through a bounded durable query ordered by oldest `created_at`, and exact trigger creation dedupe must query Postgres by `fetch_batch_id + job_item_id + source_table_name + symbol + trade_date`; workers must not load the newest 1000 triggers as a proxy for the whole backlog. In Postgres queue mode, `/source/fetch/submit` must first resolve `idempotency_key` through `governance.raw_fetch_idempotency_key_v1`; a durable hit returns the existing `fetch_batch_id` before plan expansion. New keyed batches must persist the idempotency row immediately after the batch row is created, before per-job inserts, so scheduler/data-inspector retries after API restart or client timeout do not rebuild full A-share plans. Historical duplicate request hashes must then be resolved through one bulk lookup for the whole planned job set before per-job queue writes; full A-share scheduled submits must not issue one durable duplicate query per symbol before accepting the batch.
...
Duplicate fetch submissions also check historical succeeded/skipped jobs for raw audit hashes. If a duplicate primary job is marked succeeded but has no `raw_request_hash` or no `raw_response_schema_hash`, it is treated as an unusable historical success: the new batch first tries to queue or reuse the planned backup job instead of silently skipping or creating another failed source build trigger. If the backup job already exists in a terminal unusable state for the same `provider + api + raw_table + request_hash` and cannot be reused as live work, the service queues a new auditable repair attempt for the original planned primary request with `request_params.__repair_attempt_id`; this preserves the real provider request while avoiding the request-hash uniqueness collision. The submit result must report `submitted_job_count=1` and `skipped_duplicate_count=0` for this repair path. The old job/callback/build-trigger audit is retained; the repair path appends new queue and callback evidence.
...
Stock universe market-batch repair contract:

- `source.stock_universe_daily_v1` still uses BaoStock `query_all_stock` as the normal `full_a_share` market-batch primary job; normal scheduled days do not fan out thousands of symbol jobs before the primary source has actually failed.
- If that primary market-batch job fails or returns zero rows in a P0/P1 real worker cycle, `complete_fetch_job` queues symbol-level fan-out backup jobs in the same fetch batch. Symbols are read from source facts with an internal `full_a_share` row limit of 20000 so Postgres read defaults do not truncate the stock universe at 1000 rows. The fan-out only uses A-share symbols and does not invent sample symbols.
- Fan-out backup jobs use BaoStock `query_history_k_data_plus_daily_raw` and build `source.stock_universe_daily_v1` from real `tradestatus/tradeStatus` and `isST` fields. `stock_name` may be copied from `source.stock_master_v1` when available; missing names remain absent instead of being fabricated. Unparseable trade status remains a warning/blocker and must not default to tradable.
- A repeat repair that hits a historical terminal failed market-batch request now creates an auditable repair job under the new `fetch_batch_id` instead of reporting a misleading `succeeded` batch with `job_count=0`. The old failed job remains as audit evidence through `backup_of_job_item_id` / callback payload references.
- Public `/source/rows` reads remain bounded by default. Only internal full-market planning requests the larger source-row limit required for scheduler and repair correctness.
...
9. **持久化队列恢复不依赖进程内上下文**：Postgres 队列是 fetch batch/job/callback 的事实源；worker 启动、租约前和 lease maintenance 都必须按 `job.fetch_batch_id` 从 Postgres 恢复 batch 回调上下文，并用 Postgres 中更新的 queued/retry 状态刷新 worker 进程内旧状态。若数据库中真的不存在 batch，job 必须进入 `dead_letter` 并记录 `fetch_batch_missing`，禁止因 `_BATCHES` 缺失导致 worker 重启循环或静默丢弃任务。
   2026-07-15 修复：`source-data-worker` 每次 lease 前必须先执行过期租约维护，避免 stale `leased` 占死 provider 并发；Postgres active job 装载和 source build trigger 装载在同优先级下按 `trade_date DESC NULLS LAST, created_at ASC` 取候选，确保今日日生命周期任务不会被历史 backlog 饿住。该策略不删除旧任务，旧积压仍保留在正式队列后续处理。
...
- `source.trade_status_v1.is_delisting_risk` is built only from real stock-basic status evidence (`raw_baostock.query_stock_basic_v1` primary or `raw_tushare.stock_basic_v1` backup). A real delist date or explicit non-listed status writes `true`; explicit listed status with no delist date writes `false`; missing or unparseable status remains a warning/gap. This stock-basic path must not derive `is_tradable`, `is_suspended`, `is_st`, or `raw_status`.
...
## Daily Fetch Task Lifecycle

Since 2026-07-20, scheduler-origin source fetches may carry `FetchPlanRequest.orchestration_context`. For normal `/source/fetch/submit` plans, source-data-service copies that context into each planned job under the internal `request_params.__orchestration_context` key before calculating `request_hash`. This partitions high-frequency jobs by `schedule_code`, `biz_key`, `run_slot`, `scheduled_at`, and `lifecycle_expires_at`, so 09:30 and 09:35 minute/quote snapshots are not silently collapsed as duplicate provider requests. Worker execution strips internal `__*` keys before provider calls; providers never receive scheduler metadata.

Lifecycle close is a state transition, not physical deletion and not data repair. Queue maintenance marks queued or leased scheduler jobs as `cancelled` with `last_error_code=expired_lifecycle` when `__orchestration_context.lifecycle_expires_at <= now`. For legacy same-day scheduler jobs created before this context existed, durable Postgres maintenance applies a bounded fallback by source table: intraday quote/minute/tick/auction jobs expire after 10 minutes, preopen universe/trade-status/limit-price jobs stay open until 23:59:59 of the same market day, paid probability jobs after 4 hours, and daily close/research/news jobs after 2 hours; older trade dates still close immediately. The fallback is limited to `scheduler-service` batches with `scheduled_periodic` or `model_release_preflight` trigger types, so repair/backfill/provider-probe jobs remain protected. Callback completion for an expired job cannot revive it as successful.

Queued or stale running `source_build_trigger_v1` rows linked to expired jobs are also marked `cancelled` when they have no terminal execution result. `/source/ops/daily-data-summary` exposes `raw_cancelled_jobs`, table-level `raw_cancelled_count`, and `expired_closed_table_count`; a table with only expired cancelled raw jobs and no active/waiting/source evidence is reported as `data_asset_status=expired_closed`. Missing data after lifecycle close must be repaired through a new formal source fetch/repair/backfill request.

## Worker Filtered Durable Lease Hydration

Since 2026-07-15, `source-data-worker` may be started with `SOURCE_DATA_WORKER_PROVIDERS` and `SOURCE_DATA_WORKER_QUEUE_NAMES` as comma-separated filters. These filters are not only in-memory lease filters: `lease_fetch_jobs` passes them into Postgres active-state hydration, and `PostgresQueuePersistence.load_active_state` applies them in SQL before the bounded queued-job limit. A filtered EastMoney research worker must therefore see `research_queue + eastmoney` jobs even when the newest global queue window is filled by BaoStock normal daily jobs.

This change preserves DS-5 queue ownership. Workers still lease and complete through `/source/fetch/worker/pull` and `/source/fetch/jobs/{id}/complete`; they do not bypass raw ingest, source build trigger creation, lineage, retry/dead-letter, or lifecycle close.

Compose now defines two guarded targeted worker roles in addition to the general worker: `source-data-worker-eastmoney-research` runs `providers=eastmoney, queue_names=research_queue`, and `source-data-worker-tencent-normal` runs `providers=tencent, queue_names=normal_daily_ingest_queue`. Each defaults to `max_jobs=1` per cycle so provider pressure stays bounded while different data-asset queues can make progress independently.

Since 2026-07-23, the general `source-data-worker` Compose default is `SOURCE_DATA_WORKER_MAX_JOBS=20`, while provider/API policies remain the real concurrency gate. BaoStock daily raw and qfq repair bursts are capped at `max_concurrency=12` and `requests_per_minute=300`; THS paid probability and other credentialed/public fragile endpoints keep their lower serialized limits. This keeps full A-share daily repair moving without letting worker process count bypass provider-level protection.

Same-priority same-trade-date lease candidates are interleaved by `source_table_name` within the same queue/provider/API group. Durable Postgres hydration uses a per-source-table row rank before the bounded queued-job window, and in-process leasing keeps the same round-robin order. A large `repair_queue` batch for one P0 asset therefore must not starve other P0 daily assets such as `source.daily_bar_v1` or `source.adjusted_daily_bar_v1`.

If a new higher-priority repair/preflight request hits an existing active duplicate job, source-data-service still records the alias and still reports the duplicate as skipped for compatibility, but a not-yet-leased queued job is promoted in place to the higher `priority` and `queue_name`. This prevents old P1 normal-ingest jobs from starving P0 gap repairs for the same provider/API/request hash. Already leased jobs are not preempted.

### 2026-07-23 Acceptance Probe Sample Guard

`source_data_acceptance.py --real-provider-probe` uses `sz.000063` for BaoStock `query_adjust_factor`, with `start_date=1990-01-01` and the current settled probe end date. `sz.000001` can legitimately return zero adjustment rows, so it must not be used as the required production-readiness sample for adjustment-factor schema evidence. A successful manual probe on 2026-07-23 returned `row_count=27` and restored `real_provider_probe_evidence` to passed.

### 2026-07-23 Duplicate Raw Reuse Requires Target Source Evidence

`/source/fetch/submit` no longer treats a previously succeeded raw job as reusable by raw status alone. When a duplicate succeeded raw job is found for a symbol-date source table, source-data-service now checks whether the target `source.*` row already exists or whether a non-terminal/succeeded source build trigger is still present. If neither exists, the duplicate submit queues a new auditable repair attempt with reason `duplicate_success_without_target_source_row` instead of returning `skipped_duplicate`. This prevents partial raw success from hiding missing canonical source output on the admin task board.
