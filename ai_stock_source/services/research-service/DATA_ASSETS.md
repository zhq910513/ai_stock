...
### research-service -> model-execution -> hot candidates reset audit state 2026-07-08

- 记录时间：2026-07-08 Asia/Shanghai。
- 确认来源：用户批准按范围清理模型一数据且无需备份历史数据，随后授权 Codex “你来决定”。
- 已清审计范围：`governance.research_model_execution_audit_v1` 中 `model_code='hot_candidates'`、`owner_service='hot-candidates-service'` 或 `task_code LIKE 'hot.%'` 的行，共删除 `703` 行。
- 未清审计范围：非模型一 execution audit、scheduler task store、source fetch queue、source/raw/lineage、THS Cookie 和付费概率批次状态均未清。
- 验收事实：模型一 execution audit 剩余 `0`；`decision_hot.*` 总行数 `0`；`/research/model-list/hot?limit=20` 返回 `item_count=0`；source/scheduler/data-inspector ready，source queue `queued=0/leased=0/dead_letter=0`。
- 数据边界：本记录只说明一次经授权的数据清理状态，不改变 `research_model_execution_v1` 合同。后续模型一执行仍必须由 scheduler live dispatch 或明确授权的 research execution 触发；缺 source 候选、缺付费概率或 Cookie 失效时必须落为真实 gap/blocked 状态。
- 回滚方式：本轮无历史备份，已删除的模型一 execution audit 不可从备份恢复；只能通过后续正式执行产生新的 execution audit。

### research-service -> model-execution -> t relay reset audit state 2026-07-08
...
- Cleared audit scope: `governance.research_model_execution_audit_v1` rows where `model_code='t_board_relay'`, `owner_service='t-board-relay-service'`, or `task_code LIKE 't_relay.%'`; the cleanup deleted `1905` rows.
- Preserved audit scope: non-model4 execution audit, scheduler task store, source fetch queue, source/raw/lineage, provider probe evidence, and owner service code/config were not cleared or rewritten.
- Verification facts: model4 execution audit remaining `0`; all `decision_t_relay.*` / `research_t_relay.*` rows remaining `0`; owner observation-board returns `0` items; source, scheduler, data-inspector, and t-board-relay readyz are ready.
- Boundary: this is an authorized reset state, not a contract change. Future t relay execution audit must be regenerated by formal scheduler/research execution and must not hide missing facts with `0`, empty strings, mock payloads, or inferred data.

### research-service -> model-execution -> memory and ambush reset audit state 2026-07-08
...
- Cleared model2 audit scope: `governance.research_model_execution_audit_v1` rows where `model_code='candidate_memory'`, `owner_service='candidate-memory-service'`, or `task_code LIKE 'memory.%'`; `0` rows were deleted because the scope was already empty.
- Cleared model3 audit scope: `governance.research_model_execution_audit_v1` rows where `model_code='ambush_watchlist'`, `owner_service='ambush-watchlist-service'`, `task_code LIKE 'ambush.%'`, or `task_code LIKE 'dragon.%'`; `3` rows were deleted.
- Cleared model3 research dictionary scope: `research_ambush.ambush_valley_label_taxonomy_v1`; `11` rows were deleted.
- Verification facts: model2 execution audit `0`; model3 execution audit `0`; model3 taxonomy `0`; all model2/model3 related decision/research/compatible tables have no non-zero residual rows; source, scheduler, data-inspector, and research readyz are ready; model2/model3 owner containers remain exited as requested.
- Boundary: this is an authorized reset state, not a contract change. Future model2/model3 execution audit must be regenerated by formal scheduler/research execution and must not hide missing facts with `0`, empty strings, mock payloads, or inferred data.
