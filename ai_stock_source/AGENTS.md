...
2026-07-02 模型四终止 / 缺口默认下架修复后，用户明确回复“拍板”批准冻结：`t-board-relay-service -> observation-board -> terminal data gap and stale result guard` 与 `shence-frontend-service -> model-tboard -> terminal default visibility` 正式冻结。冻结证据为模型四 owner `/t-board-relay/observation-board?limit=20` 中 `000823.SZ` 投影为 `observation_status=data_wait`、`model_score=NULL`、真实阶段事实时间保持 `2026-06-26T07:53:37.143354+00:00`、最后模型产出时间为 `2026-07-02T07:02:00+00:00`；前端默认 `/api/model-list/tboard?limit=20` 返回 0 条普通用户可见观察对象，`include_stale_stopped=true` 仍可审计查询 5 条历史 / 缺口对象；模型四 owner 测试 `30 passed`，前端合同测试 `20 passed`，Python 编译、JS 语法和 `git diff --check` 均通过；scheduler、data-inspector、frontend ready，`source-data-service` 未重启。冻结后未经解锁不得把 Day2 窗口已过且缺有效五分钟监测事实的对象显示为 `continue_watch`，不得用历史 30 分钟 `model_result_30m` 快照复活 `data_wait/stopped/completed` 对象的旧状态、旧分数或旧风险结论，不得让前端默认列表展示 `stopped` 终止对象或 Day2 窗口已过的 `data_wait` 对象；`include_stale_stopped=true` 只能作为只读审计排查参数，不得删除、改写或截断 owner append-only 事实。
