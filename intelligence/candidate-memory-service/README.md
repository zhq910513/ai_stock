...
状态：migrated_copy
来源：ai_stock_source/services/models_services/candidate-memory-service
...
### candidate-memory-service -> model2 reset -> 2026-07-08 zero state
...
- Cleared scope for model2: `governance.research_model_execution_audit_v1` rows where `model_code='candidate_memory'`, `owner_service='candidate-memory-service'`, or `task_code LIKE 'memory.%'`; the table already had `0` matching rows, so no model2 audit row was deleted.
- Current data state: all current `decision_memory.*`, `research_memory.*`, and compatible `decision.candidate_memory*` tables have `0` rows.
- Runtime state: `ai-stock-candidate-memory-service` remains `Exited (0)` as requested; source, scheduler, data-inspector, and research services remain ready.
- Boundary: this is an authorized zero-data operational state only. Future candidate-memory facts must be regenerated by the formal scheduler/research/owner chain and must preserve real `NULL`, gap codes, blocked states, and append-only semantics.

### candidate-memory-service -> model2 reset -> zero-data state freeze 2026-07-08

- 冻结对象：`candidate-memory-service -> model2 reset -> zero-data state`。
- 冻结时间：2026-07-08 Asia/Shanghai。
- 拍板人 / 确认来源：用户回复“你来决定拍板”；Codex 基于本轮数据库计数、服务健康和 owner 未启动状态决定可以拍板冻结。
- 锁定范围：模型二当前零数据运行状态；`decision_memory.*`、`research_memory.*` 和兼容 `decision.candidate_memory*` 表均为 `0` 行；模型二 research execution audit 为 `0` 行；`ai-stock-candidate-memory-service` 按用户要求保持 `Exited (0)`，本轮不启动。
- 允许的只读验收：数据库只读计数、`governance.research_model_execution_audit_v1` 模型二过滤计数、Docker 只读状态、source/scheduler/data-inspector/research readyz。
- 禁止修改项：未经解锁不得手工写入模型二事实表、不得用 `0`、空字符串、mock payload 或推断补缺口、不得绕过 scheduler/research/owner 正式链路生成模型二事实、不得借本冻结记录修改模型二阈值/状态机/调度/接口。
- 解锁条件：用户明确批准启动模型二 owner、恢复模型二正式调度/执行、重建模型二事实、恢复历史数据或修改模型二数据合同。
- 回滚方式：本轮无历史备份；已清空/已为空状态不能从本轮备份恢复。若需要恢复，只能通过正式 scheduler/research/owner 链路重新生成事实；文档冻结记录可按 Git 差异回退。
- 验证清单：模型二相关表非零残留为 0；模型二 execution audit 为 0；核心服务 ready；candidate-memory owner 仍未启动。
